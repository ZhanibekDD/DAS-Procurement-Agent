...
НДС, позиции, количество, цену и проверяемый номер исходной строки. Для
многоуровневых матриц каждая позиция содержит `offers`: поставщика, вид цены
(`work`, `materials_project`, `materials_analogue`), цену за единицу, сумму и
номер исходной колонки. Предпросмотр ничего не записывает в закупку: результат
обязательно подтверждает сотрудник.
...
Ограничения первой версии: `.xlsx` и UTF-8 `.csv` до 25 МБ. Матрица цен
привязывается автоматически только когда поставщики явно указаны над своими
блоками колонок. Если заголовок нельзя доказуемо сопоставить, позиции остаются
без `offers` и помечаются предупреждением вместо потенциально неверного решения.
